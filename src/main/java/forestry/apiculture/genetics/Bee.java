...
public class Bee extends IndividualLiving implements IBee {
	...
	public Set<IErrorState> getCanWork(IBeeHousing housing) {
		...
		/*if (genome.getPrimary().getRequiredBiomeNames() != null && !Arrays.asList(genome.getPrimary().getRequiredBiomeNames()).contains(biome.biomeName))
		{
		...
		if (!housing.doesBlockHaveOxygen())
		{
			errorStates.add(EnumErrorCode.NO_OXYGEN);
		}
